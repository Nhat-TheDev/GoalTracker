...
- **Brainstorm before you draft, when there's a real decision to make — if you have it.** `superpowers:brainstorming` ships separately, as part of the optional Superpowers skill collection — it is not bundled with GoalTracker, and this is a recommendation, not a requirement. If it's available in your environment, and the goal is large or ambiguous enough that more than one real approach exists — not just "which exact wording for the criteria," but genuine trade-offs in direction — invoke it before writing `overview`, rather than drafting the first approach that comes to mind. Fold the chosen direction into `overview`/`acceptance_criteria`, and fold why the other approaches were rejected into `constraints`/`out_of_scope`, so that reasoning survives past this conversation instead of living only in the brainstorm transcript. Skip this — and just draft directly, per the next bullet — when the skill isn't installed, or the goal's approach is already obvious; brainstorming a one-way-door-free bug fix is wasted motion either way.
- **Draft before you commit.** Work out a first-pass `overview`, `acceptance_criteria`, `constraints`, and `out_of_scope` from what you already know (or from the brainstorm above), then present it to the user in the **spec draft** format below — *before* treating `spec_set` as final. Don't silently assume what "done" means and move straight to milestones — state it, in a consistent structure, and let them correct it.
...
- **Before calling `task_update_status(id, "completed")`, make sure at least one `task_add_note(type="evidence")` exists on that task.** This isn't schema-enforced — nothing stops you from skipping it — so treat it as a discipline check you run on yourself every time, not a gate the MCP will catch for you.
  - Minimum evidence bar, in order of preference: unit test output, a coverage delta, or a passing CI/typecheck run relevant to the change — cite the actual command and result (e.g. "`npm test` — 42/42 pass, new test `rejects X` added"), not just "tests pass."
  - If none of those genuinely apply (a documentation-only change, a config tweak with nothing to run), evidence is still required — add a `task_add_note(type="evidence")` stating plainly why none applies, rather than marking the task done in silence.
  - For UI/visual changes where the real verification is looking at a screen: save the screenshot or recording to `~/.goaltracker/evidences/<goal_id>/` and reference the file path in the note's `content`. This is a filesystem convention for you to follow, not a GoalTracker tool or capability — the MCP never touches the filesystem beyond its own DB file, so nothing creates this folder or validates what's in it.
